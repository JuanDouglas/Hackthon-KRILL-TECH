...
export const AlertaPrecoceRJView: React.FC<AlertaPrecoceRJViewProps> = ({
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto pb-10">
      ...
      <div className="bg-[#09090e] border border-[#231c3a] rounded-3xl p-6 sm:p-7 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-xs text-slate-500 font-display">Alerta Precoce de Insolvência</span>
          </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="bg-[#0e0e17] p-4 rounded-2xl border border-rose-500/30 text-right">
          ...
          {onOpenTestBench && (
            <button
              ...
              className="px-4 py-3 rounded-2xl bg-[#141422] hover:bg-[#6618F7] text-[#a78bfa] hover:text-white border border-[#231c3a] hover:border-[#6618F7] text-xs font-display font-bold flex items-center gap-2 transition-all cursor-pointer h-full"
      ...
      <div className="bg-[#09090e] border border-[#231c3a] rounded-2xl p-5 shadow-xl space-y-3">
        <div className="flex items-center justify-between pb-2 border-b border-[#231c3a]">
          ...
          <span className="text-xs text-slate-400 font-display">Ação Imediata</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="p-3 rounded-xl bg-[#0e0e17] border border-[#231c3a] space-y-0.5">
            <span className="text-[10px] font-display font-bold text-[#8b4dff] uppercase">Passo 1 • T + 0h</span>
          ...
          <div className="p-3 rounded-xl bg-[#0e0e17] border border-[#231c3a] space-y-0.5">
            <span className="text-[10px] font-display font-bold text-[#8b4dff] uppercase">Passo 2 • T + 4h</span>
          ...
          <div className="p-3 rounded-xl bg-[#0e0e17] border border-[#231c3a] space-y-0.5">
            <span className="text-[10px] font-display font-bold text-[#8b4dff] uppercase">Passo 3 • T + 12h</span>
      ...
      <div className="bg-[#09090e] border border-[#231c3a] rounded-2xl p-6 shadow-2xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#231c3a]">
          <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-display font-medium transition-all cursor-pointer ${
                selectedFilter === 'ALL'
                  ? 'bg-[#6618F7] text-white font-bold shadow-md shadow-[#6618F7]/30'
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-display font-medium transition-all cursor-pointer ${
                selectedFilter === 'PERIODIC'
                  ? 'bg-[#141422] text-white font-bold border border-[#6618F7]'
                  : 'bg-[#0e0e17] text-slate-400 border border-[#231c3a] hover:text-white'
              }`}
            >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg font-display font-medium transition-all cursor-pointer ${
                  selectedFilter === 'SIMULATED'
                    ...
                    : 'bg-[#0e0e17] text-purple-300 border border-purple-500/40 border-dashed hover:text-white'
                }`}
              >
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] font-display text-slate-500">{alert.timestamp}</span>
                  </div>
                ...
                <div className="bg-[#000000] p-3 rounded-xl border border-[#231c3a] text-[11px] space-y-1.5">
                  <div className="flex justify-between items-center font-sans">
                    <span className="text-slate-500">Tomador com Exposição:</span>
                    <span className="font-bold text-white font-display">{alert.borrowerName}</span>
                  </div>
                  <div className="flex justify-between items-center font-sans">
                    <span className="text-slate-500">Degradação no Score:</span>
                    <span className="font-display font-bold text-rose-400">
                      {alert.scoreImpact} pts (Rating {alert.previousRating} → {alert.newRating})
                    </span>
                  </div>
                  <div className="pt-1.5 border-t border-[#231c3a] text-slate-300 font-sans leading-relaxed">
